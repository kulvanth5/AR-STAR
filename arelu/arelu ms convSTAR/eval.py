...
def evaluate_fieldwise(model, model_gt, dataset, batchsize=1, workers=8, viz=False, fold_num=5, level=3,
    ...
    print_report(*confusion_matrix_to_accuraccies(confusion_matrix))
    np.savez('/kaggle/working/arelu_CMs/preds_gt/' + str(
                level) + '_fold_' + str(fold_num), predictions=prediction_wo_fieldwise_refined,targets=targets_wo_unknown)
    ...
    if viz:
        ...
        if level == 3:
            np.savez('/kaggle/working/arelu_CMs/' + str(
